...
main(int argc, char* argv[])
{
    ...
    Config::SetDefault("ns3::TcpLlm::TopolgyDescription", StringValue("Current network topology with three senders S0 S1 S2, two routers R1 R2, and a receiver, where: Links from sender S0 and S1 to R1 have a bandwidth of 1000 Mbps with a delay of 5 ms respectivaly; Link between R1 and R2 (the bottleneck) has a bandwidth of 15 Mbps and a delay of 10 ms; Link from Sender S2 to R2 has a bandwidth of 1000 Mbps with a delay of 5 ms; Link from R2 to the receiver has a bandwidth of 1000 Mbps with a delay of 5 ms."));
    ...
    // Modified! Sender 2 is connected to R2
    NetDeviceContainer sender2Edge = edgeLink.Install(senders.Get(2), routers.Get(1));
